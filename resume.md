...
**运营活动**，背景是咱们经常有一些酒店促销所以需要配合一些主题活动。主要的挑战是**兼容IE6和因为活动需要加载大量的图片资源**，需要作一些简单的性能优化。对于兼容性我的办法是定期收集整理。图片性能优化主要是通过压缩，懒加载。以及总结的业务经验（比如头部banner比较重要，压缩不要太狠，背景以及中部图片，没那么重要可以适当使劲儿压缩），在保证效果情况下，使用有损压缩jpg。以及一些新的图片格式webp等探索。
