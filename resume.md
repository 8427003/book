...
<!-- timeline -->
## 2016-09（百度贴吧）
主要参与了**pc端bigpipe迁移**和**hybrid高级抽奖帖**项目。
####**pc端bigpipe迁移**
...
主要收获是：这种大规模迁移需要屏蔽风险，提前预估工作量，投入人力计算。注意迁移过程中同步更新需要有效管理。另外深入理解了bigpipe具体实现。

####**hybrid高级抽奖帖**
**背景**是在手机端魅族吧里定制一种抽奖贴子。
我的**职责**是用h5实现一个老虎机，让端嵌入到他们的native应用中。主要的挑战是**hybrid调试**，与**native部门的合作**上，以前手机端动画性能。对于前两个问题都没有作好。

后来总结如下：1. 因为没有调试机制导致联调周期变长（一定要先排出项目难点，确保周期准确性）。2.跨部门合作时需要先了解对方的开发流程。比如native是先进行ui走查，再提测。我们是先提测再ui走查。导致ui走查bug算在业务bug中，bug率高。3.手机动画性能上，由于手机本身cpu性能不高，即使是canvas也没有css3动画表现得流畅。所以动画能用css3时尽量用css3而非js。
...
主要职责是**运营活动页**开发和**独立负责电影票后台系统**。

**运营活动**，背景是咱们经常有一些酒店促销所以需要配合一些主题活动。涉及的**技术栈**主要是：**jquery，css3**等。主要的挑战是**兼容IE6和因为活动需要加载大量的图片资源**，需要作一些简单的性能优化。对于兼容性我的办法是**定期收集整理**。图片性能优化主要是通过**压缩，懒加载**。以及总结的业务经验（比如头部banner比较重要，压缩不要太狠，背景以及中部图片，没那么重要可以适当使劲儿压缩），在保证效果情况下，使用有损压缩jpg。以及**一些新的图片格式webp等探索**。

**电影票后台系统**是一个全新的系统，包括活动，排片管理，影院管理，报价等重要模块。技术栈是：**vue**，**bootstrap**。涉及到一些**技术选型**跟前端同学交流。另外主动了解后端系统关键实现，跟后端同学请教，**深入到数据库结构设计**。了解电影票整体业务（包括有意思的制片人，导演，发行商，影院，院线等收益分成）。以电影业务为主题作**团队内部分享1次**。
