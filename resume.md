...
**运营活动**主要的挑战是兼容IE6和因为活动需要加载大量的图片资源，需要作一些简单的性能优化。对于兼容性我的办法是定期收集整理。图片性能优化主要是通过压缩，懒加载。以及总结的业务经验（比如头部banner比较重要，压缩不要太狠，背景以及中部图片，没那么重要可以适当使劲儿压缩），在保证效果情况下，使用有损压缩jpg。以及一些新的图片格式webp等探索。

**电影票后台系统**是一个全新的系统，包括活动，排片管理，影院管理，报价等重要模块。涉及到一些技术选型跟前端同学交流。另外主动了解后端系统关键实现，跟后端同学请教，深入到数据库结构设计。了解电影票整体业务。以电影业务为主题作团队内部分享1次。
